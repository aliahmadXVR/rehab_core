...
from rehab_msgs.msg import bumper, WheelsCmdStamped
...
navigation_point =  [5.47, -0.94, 0.338, 0.94] #its the goal_point in front of docking station convention=x,y,th,w
...
def press_both_bumpers():
    print("bpth bumpers")
    if (bumper_id == 1): #left bumper pressed
        while( not rospy.is_shutdown() ):
            #print("both 1st")
            wheels_cmd_msg.wheels_cmd.angular_velocities.joint = [0.0, -0.3]
            if(bumper_id == 5):
                #print("both 2nd")
                break
    ...
    elif (bumper_id == 5): #right bumper pressed
        while( not rospy.is_shutdown() ):
            wheels_cmd_msg.wheels_cmd.angular_velocities.joint = [-0.3, 0.0]
            #print("both 3rd")
            if(bumper_id == 1):
                #print("both 4th")
                break
...
if __name__ == '__main__':
    ...
    rospy.init_node('both_limit_switch_stopping')
